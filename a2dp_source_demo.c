...
#define A2DP_SOURCE_DEMO_PREFERRED_SAMPLING_RATE 48000
...
static uint8_t media_sbc_codec_capabilities[] = {
  (AVDTP_SBC_44100 << 4) | (AVDTP_SBC_48000 << 4) | AVDTP_SBC_DUAL_CHANNEL,
...
// Minijambox:
static const char *device_addr_string = "B4:0E:CF:2B:01:44";
...
static int a2dp_demo_fill_sbc_audio_buffer(a2dp_media_sending_context_t *context)
{
  ...
  while (context->samples_ready >= num_audio_samples_per_sbc_buffer && (context->max_media_payload_size - context->sbc_storage_count) >= sbc_buffer_length) {
...
static void a2dp_source_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
  ...
  switch (hci_event_a2dp_meta_get_subevent_code(packet)) {
    ...
    case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_OTHER_CONFIGURATION:
      printf("recv OTHER_CONFIGURATION\n");
      break;
