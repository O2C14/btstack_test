...
static void handle_l2cap_media_data_packet(uint8_t seid, uint8_t *packet, uint16_t size)
{
  ...
  send_to_decoder(packet+12, size-12);
}
...
static void a2dp_sink_packet_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
  ...
  switch (packet[2]) {
    case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_OTHER_CAPABILITY:
      printf("recv OTHER_CAPABILITY\n");
      break;
    case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_OTHER_CONFIGURATION:
      printf("A2DP  Sink      : Received non SBC codec - not implemented\n");
      init_decoder(a2dp_subevent_signaling_media_codec_other_configuration_get_local_seid(packet));
      break;
    case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_SBC_CONFIGURATION: {
      printf("A2DP  Sink      : Received SBC codec configuration\n");
      init_decoder(a2dp_subevent_signaling_media_codec_sbc_configuration_get_local_seid(packet));
      break;
    }
    case A2DP_SUBEVENT_SIGNALING_MEDIA_CODEC_MPEG_AAC_CONFIGURATION: {
      printf("A2DP  Sink      : Received MPEG AAC codec configuration\n");
      init_decoder(a2dp_subevent_signaling_media_codec_mpeg_aac_configuration_get_local_seid(packet));
      break;
    }
